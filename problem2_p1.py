...
video=cv2.VideoCapture(file_location+"/road_track.avi")

out = cv2.VideoWriter('road_video.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 60, (1392,512))
...
src_pts=np.array([[40,230],[926,230],[530,32],[737,32]])
...
while True:
    ...
    #ROI
    blur_s=blur[int(m/2):][:][:]
    blur_untouched=blur[0:int(m/2)][:][:]
    cv2.imshow("blur_u",blur_untouched)
    blur=blur_s
    cv2.imwrite("blur1_r.jpg",blur)
    ...
    elif grad_avg>-0.01 and grad_avg<=0.01:
        print("straight")
